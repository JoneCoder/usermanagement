...
class Admin extends Authenticatable
{
    ...
    public function getTotalDayPresentAttribute($value)
    {
        $now = Carbon::now();
        $monthlyPresent = Attendance::where('admin_id', $this->id)->whereMonth('login_time', $now->month)->whereYear('login_time', $now->year)->count();
        return $monthlyPresent?? 0;
    }
